...
- AI 초안은 `[핵심 메시지를 담은 소제목]`으로 시작해 훅(직무·산업 관찰) → 본론(구체 경험·수치·기업 정보 연계) → 결론(단기·장기 다짐) 구조의 에세이 형태로 작성되며, 설정된 글자 수·byte 제한의 최소 90%까지 채우도록 시도(부족하면 자동으로 한 번 더 보강). 해시태그 형식을 요구하는 문항은 자동으로 감지해 `#키워드` 형식으로만 작성
